/**
  ...
  ******************************************************************************
	* 学习笔记：
	* 按位与即&：如果两个相应的二进制位都为1，则该位的结果值为1，否则为0，作用为1.清零 2.取一个数中某些指定位 3.保留指定位
	* 按位或即|：如果两个相应的二进制位中只要有一个为1，该位的结果值为1，作用为按位或运算常用来对一个数据的某些位定值为1
	* 按位异或^：若参加运算的两个二进制位值相同则为0，否则为1，作用1.使特定位翻转（与00001111，后四位反转） 
  */
...
void Data_Send_Status(float Pitch,float Roll,float Yaw)
{
	...
	data_to_send[_cnt++]=0xAA;//i++为先赋值再加，++i为先加再赋值
 ...
 */
static void read_from_mpl(void)//从陀螺仪中读取数据
{
    long msg, data[9];
    int8_t accuracy;
    ...
    if(1)
    {
				...
				/*获取欧拉角*/
			  if (inv_get_sensor_type_euler(data, &accuracy,(inv_time_t*)&timestamp_1))
						{
																		
						#ifdef USE_USART_DISPLAY
									sprintf ( cStr_1, "Pitch :  %.4f  ", data[0]*1.0/(1<<16) );	//inv_get_sensor_type_euler读出的数据是Q16格式，所以左移16位.
									//LCD_DisplayStringLine(LINE(7),(uint8_t* )cStr_1);			
									printf("%s \n", (uint8_t* )cStr_1); //输出俯仰角
									
									sprintf ( cStr_1, "Roll  :  %.4f  ", data[1]*1.0/(1<<16) );	//inv_get_sensor_type_euler读出的数据是Q16格式，所以左移16位.
									//LCD_DisplayStringLine(LINE(8),(uint8_t* )cStr_1);	
									printf("%s \n", (uint8_t* )cStr_1); //输出翻滚角
									
									sprintf ( cStr_1, "Yaw   :  %.4f  ", data[2]*1.0/(1<<16) );	//inv_get_sensor_type_euler读出的数据是Q16格式，所以左移16位.
									//LCD_DisplayStringLine(LINE(9),(uint8_t* )cStr_1);	
									printf("%s \n", (uint8_t* )cStr_1); //输出偏航角
									
									/*温度*/
									mpu_get_temperature(data,(inv_time_t*)&timestamp_1); 
									
									sprintf ( cStr_1, "Temperature  :  %.2f  ", data[0]*1.0/(1<<16) );	//inv_get_sensor_type_euler读出的数据是Q16格式，所以左移16位.
									//LCD_DisplayStringLine(LINE(10),(uint8_t* )cStr_1);
									printf("%s \n", (uint8_t* )cStr_1); 
						#endif
					...
					/*获取步数*/        
				get_tick_count(&timestamp_1);
				if (timestamp_1 > hal.next_pedo_ms) {

						hal.next_pedo_ms = timestamp_1 + PEDO_READ_MS;
						dmp_get_pedometer_step_count(&step_count_1);
						dmp_get_pedometer_walk_time(&walk_time_1);				

						#ifdef USE_USART_DISPLAY
									sprintf(cStr_1, "Walked steps :  %ld  steps over  %ld  milliseconds..",step_count_1,walk_time_1);
									//LCD_DisplayStringLine(LINE(11),(uint8_t* )cStr_1);	
									printf("%s \n", (uint8_t* )cStr_1); 
						#endif				
				}
			}	
...
static void handle_input(void)
{
    ...
    case 'a':
        hal.report ^= PRINT_ACCEL;//Prints Accel Data,加速度数据
        break;
    case 'g':
        hal.report ^= PRINT_GYRO;//Prints Gyro Data,打印陀螺仪数据
        break;
#ifdef COMPASS_ENABLED
    case 'c':
  ...
  if (result) {
      MPL_LOGE("Could not initialize gyro.\n");
			LED_RED;
							...
							printf("Please check the hardware connection!\n");
				#endif
  }
	else
	{	
